...
class GetDistances():
    """
      Gets clustered generated by CD-Hit generates multiple sequence alignment using clustalo
      and calculates a distance matrix of pairwise genetic distances 
    """
    ...
    def  __init__(self):
        ...
        parser.add_argument('-s','--subsampling', default = 100000, help="""subsampling depth limit for pairwise distances to be analysed.""", required=False, type=int)
        parser.add_argument('-c','--cluster_id', default = [0.97, 1.0], nargs='+', help="""clustering sequence identity percentage""", required=False, type=float)
    ...
    def get_dist(self, file_in):
        """
           Gets the CD-Hit file and submits it to clustalo to generate alignment
        """
        strip_fname = lambda fname: float(fname.split('r_')[-1].replace('.cd_hits','')) 
        if strip_fname(file_in) not in self.cluster_id:
        ...
        #pprint.pprint(self.distance_arrays)
    ...
    def parse_dist(self, dist_fname):
        ...
        try:
            with open(dist_fname) as dist_file:
                ...
                #print(n_array)
                if (n_array > self.subsampling):
                    dist_matrix = np.random.choice(dist_matrix, self.subsampling,  replace=True)            
                self.distance_arrays[dist_fname] = dist_matrix
                #saving the length of the sequences
                self.data_len[dist_fname] =  i
                #print(self.data_len[dist_fname])
    ...
    def Init_plot(self, cluster_data, plt_fname, lens, top = 3):
        ...
        plot = axs.boxplot(dist_data,
                           positions=sorted(locations.values()),
                           patch_artist=True,
        ...
        axs.set_xticklabels(labels, fontsize=10, rotation=30, ha='right')
        axs.yaxis.set_tick_params(labelsize=10)
        
        plt.ylabel('Pairwise genetic distance', fontsize=14, weight = 'bold')
        plt.xlabel('Primer pair', fontsize=14, weight = 'bold')
        ...
        if n_clusters > 2:
            for i,j in it.combinations(top_labels, 2):
                print "({},{})".format(i,j),
